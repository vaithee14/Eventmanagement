...
export default function Reception() {
  ...
  useEffect(() => {
    const FetchReceptionEvents = async () => {
      try {
        const receptionEvents = await axios.get(
           "http://localhost:4050/reception/getEvents"
        );
        SetreceptionEvent(receptionEvents.data);
      } catch (error) {
        console.log(error);
      }
    };
    FetchReceptionEvents();
  }, []);

  const addReceptionEvents = async () => {
    try {
      const newEvent = { title, description, image };
      const response = await axios.post(
        "http://localhost:4050/reception/addEvents",
        newEvent
      );
